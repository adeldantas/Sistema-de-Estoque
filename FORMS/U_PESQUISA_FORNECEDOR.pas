...
procedure TFrm_Pesquisa_Fornecedor.bt_imprimirClick(Sender: TObject);
VAR CAMINHO: STRING;
begin
  caminho:=extractfilepath(application.ExeName);
  frm_pesquisa_padrao:=Tfrm_pesquisa_padrao.Create(self);
  if frm_pesquisa_padrao.Rel_pesquisa_padrao.LoadFromFile(caminho + 'rel_Fornecedor.fr3') then
    begin
      rel_pesquisa_padrao.Clear;
      rel_pesquisa_padrao.LoadFromFile(extractfilepath(application.exename) + 'rel_Fornecedor.fr3');
