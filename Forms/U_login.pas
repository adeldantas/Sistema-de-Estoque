...
begin
  ...
  if (dm.Q_login.RecordCount>0) and (dm.Q_loginTIPO.AsString <>'OPERADOR CAIXA') Then
   ...
   if contagem >=3 then
       begin
         Messagedlg('Você ultrapassou o limite de tentativas,Fechando a aplicação!',MtInformation,[mbOk],0);
         application.Terminate;
       end;
...
// ESCOLHE O BANCO A SER UTILIZADO
 case cb_banco.ItemIndex of
    0:begin
       // conecta ao banco produção estoque
       dm.conexao.Connected:=false;
       dm.conexao.Params.Values['Database']:=
       'C:\sistema_de_estoque\EXE\BANCO\ESTOQUE.FDB';
       dm.conexao.Connected:=true;
     end;
   ...
   1:begin
       ...
       dm.conexao.Connected:=false;
       dm.conexao.Params.Values['Database']:=
       'C:\sistema_de_estoque\EXE\BANCO\TREINA.FDB';
       dm.conexao.Connected:=true;
     end;
  ...
  2:begin
       ...
       dm.conexao.Connected:=false;
       dm.conexao.Params.Values['Database']:=
       'C:\sistema_de_estoque\EXE\BANCO\TESTE.FDB';
       dm.conexao.Connected:=true;
...
begin
   // enter ter a função da tecla tab
    if Key=#13 then
       begin
         key:=#0;
