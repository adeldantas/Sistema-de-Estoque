...
uses U_DM, U_PESQ_COMPRA, U_pesq_fornecedor, U_Pesq_Formas_pgto, U_pesq_produto;
...
procedure TFrm_Compra.bt_busca_produtoClick(Sender: TObject);
begin
...
begin
  ...
  Q_conta_pagar.Open;
  parcela:=1;
   //QUANDO É A VISTA OU CARTÃO DE DEBITO
  ...
  /// SE FOR A CREDITO OU A PRAZO, NOTA PROMISSÓRIA
     Q_conta_pagar.First;
    while parcela <=Q_padraoCOND_PGTO.AsInteger do
      begin
        // ABRE PARA INSERÇÃO
        ...
        Q_conta_pagar.FieldByName('dt_vencimento').Value:=
      ...
      Q_conta_pagar.FieldByName('valor_parcela').AsFloat;
      // SE A SOMA DAS PARCELAS FOR MAIOR QUE O VALOR TOTAL
      if soma > Q_padraoVALOR.AsFloat then
        begin
          ...
          // Coloca o  novo valor na valor pagar
          Q_conta_pagar.FieldByName('total_pagar').AsFloat:=
          Q_conta_pagar.FieldByName('valor_parcela').AsFloat;
...
procedure TFrm_Compra.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
